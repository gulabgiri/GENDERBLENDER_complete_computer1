...
void testApp::draw(){
	...
	//////// CAMBIAR ESES VALORES PARA AJUSTAR IMAGEN
	...
	/*
void testApp::drawLeft() {	
	ofSetColor(255);
	ofFill();
	ofPushMatrix();		
		ofTranslate(viewOffsetX,viewOffsetY+y_offset, 0); //move to top left image corner			
		ofRotate(RadToDegree(roll+rollIni), 0, 0, 1); //rotate from centre	
		left.draw(-camWidth/2,-camHeight/2);
	ofPopMatrix();				
}

void testApp::drawRight() {
	ofSetColor(255);
	ofFill();		
	ofPushMatrix();		
		ofTranslate(viewOffsetX+x_offset,viewOffsetY+y_offset, 0);
		ofRotate(RadToDegree(roll+rollIni), 0, 0, 1);
		right.draw(-camWidth/2,-camHeight/2);
	ofPopMatrix();
